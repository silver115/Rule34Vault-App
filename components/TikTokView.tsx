...
interface TikTokViewProps {
  ...
  onActionStateChange?: (
    postId: number,
    state: { isLiked: boolean; isBookmarked: boolean; isSuperLiked: boolean },
  ) => void;
...
export function TikTokView({
  ...
  const [isSuperLiked, setIsSuperLiked] = useState(
    initialActionState?.isSuperLiked ?? false,
  );
  // keep ref in sync
  useEffect(() => {
  ...
  const [isBookmarked, setIsBookmarked] = useState(
    initialActionState?.isBookmarked ?? false,
  );
  ...
  useEffect(() => {
    ...
    initialActionState?.isLiked,
    initialActionState?.isSuperLiked,
    initialActionState?.isBookmarked,
    post.id,
  ]);
  ...
  const naturalComicHeight = isTallImage
    ? Math.round(SW * (post.height / post.width))
    : H;
  ...
  const flushAttention = useCallback(() => {
    ...
    durationSec = Math.min(
      durationSec,
      post.type === 1 ? DURATION_CAP_SEC : IMAGE_MAX_SEC,
    );
    if (durationSec >= 3) {
      sendAttentionSignal(
        token,
        ...
        post.tags ?? [],
        durationSec,
        peakCompletionRef.current,
        isLikedRef.current,
        replayCount.current,
      );
    } else if (durationSec >= 0.5) {
      // Quick scroll (<3s but >0.5s) = mild negative signal — user saw it and passed
      sendRecSignal(token, post.id, "skip", post.tags ?? []);
    }
  ...
  const userAvatarUrl = user
    ? getAvatarUrl(user.id, user.avatarModifyDate, 128)
    : null;
  ...
  useEffect(() => {
    if (isActive) {
      if (isVideo) {
        ...
        startSegment();
      } // images: start counting immediately
    } else {
      if (isVideo) {
        videoPause();
        setIsPlaying(false);
  ...
  const onPlaybackStatus = useCallback(
    (s: any) => {
      ...
      if (dur > 0) {
        ...
        if (completion > peakCompletionRef.current)
          peakCompletionRef.current = completion;
        // Detect replay: position resets to near start while previously past 10%
        if (
          lastPositionRef.current > dur * 0.1 &&
          pos < dur * 0.05 &&
          s.isPlaying
        ) {
  ...
  const handleLike = useCallback(async () => {
    ...
    try {
      ...
      onActionStateChange?.(post.id, {
        isLiked: next,
        isSuperLiked: next ? isSuperLiked : false,
        isBookmarked,
      });
    } catch {
      setIsLiked(!next);
  ...
  const progress =
    duration > 0 && !isNaN(duration) && !isNaN(position)
    ...
    <View style={[styles.container, { width: SW, height: H }]}>
            ...
            <video
              ...
              onTimeUpdate={(e: any) => {
                ...
                if (v.duration > 0) {
                  ...
                  if (c > peakCompletionRef.current)
                    peakCompletionRef.current = c;
                }
              }}
              onError={() => {
                setMediaError(true);
      ...
      {isVideo && !mediaError && (
        <Animated.View
          style={[
            ...
            { opacity: showControls ? controlsAnim : 0.0 },
          ]}
      ...
      <View style={[styles.rightSide, { bottom: 80 }]} pointerEvents="box-none">
        ...
        >
          ...
          />
          <Text
            style={[styles.sideBtnText, isSuperLiked && { color: "#FFD700" }]}
          >
            {likeCount}
          </Text>
        </Pressable>
      ...
      <View
        style={[styles.postInfo, { bottom: isVideo ? 102 : 20 }]}
        pointerEvents="none"
      >
      ...
      >
        ...
        <View style={cStyles.sheet}>
          ...
          <View style={cStyles.header}>
            <Text style={cStyles.headerTitle}>
            ...
            <FlatList
              ...
              renderItem={({ item: c }) => {
                ...
                const timeAgo = (() => {
                  const diff = Math.max(
                    0,
                    Date.now() - new Date(c.created).getTime(),
                  );
                    ...
                    <View style={cStyles.commentBody}>
                      ...
                      {c.likes > 0 && (
                        <View style={cStyles.commentLikes}>
                          <Text style={cStyles.commentLikesText}>
                            ♥ {c.likes}
                          </Text>
                        </View>
                      )}
