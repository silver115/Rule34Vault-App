...
import {
    detectSpamComments,
    fetchSharedBlocklist,
    getDistributedScanStatus,
    isBlocklisted,
    reportSpamToServer,
    resetSpamCache,
    runDistributedScanCycle
...
function Avatar({
  ...
  const uri = avatarModifyDate
    ? getAvatarUrl(userId, avatarModifyDate, size > 64 ? 256 : 128)
    : null;
...
// Cache for e621 post thumbnail URLs so we don't re-fetch every render
const e621ThumbCache = new Map<number, string>();
...
function PostImage({ postId, width }: { postId: number; width: number }) {
  ...
  const [src, setSrc] = useState(() => {
    if (isE621) return e621ThumbCache.get(postId) ?? "";
    return cdnUri;
  });
  const [err, setErr] = useState(false);
  const [loading, setLoading] = useState(isE621 && !e621ThumbCache.has(postId));
  const triedFallback = useRef(false);
  const h = Math.round(width * 0.56);

  // Fetch e621 post thumbnail URL on mount
  useEffect(() => {
    if (!isE621 || e621ThumbCache.has(postId)) return;
    let cancelled = false;
    api
      .getPost(postId)
      .then((post) => {
        if (cancelled) return;
        const url = getE621MediaUrl(post, "thumb");
        if (url) {
          e621ThumbCache.set(postId, url);
          setSrc(url);
        } else {
          setErr(true);
        }
  ...
  if (Platform.OS === "web") {
    return (
      <img
        ...
        onError={() => {
          if (!isE621 && !triedFallback.current) {
...
function SpamPanel({
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${PUSH_URL}/api/spam/groups`)
        .then((r) => r.json())
        .catch(() => null),
    ...
    async (norm: string) => {
      ...
      try {
        const resp = await fetch(
          `${PUSH_URL}/api/spam/group/${encodeURIComponent(norm)}`,
        );
  ...
  return (
    <View style={[spamStyles.panel, { backgroundColor: colors.bgCard }]}>
      {/* Header */}
      <View
        style={[spamStyles.panelHeader, { borderBottomColor: colors.border }]}
      >
        <View style={spamStyles.panelHeaderLeft}>
          <Ionicons name="shield-checkmark" size={18} color={Colors.accent} />
          <View>
            <Text style={[spamStyles.panelTitle, { color: colors.text }]}>
              Spam Filter
            </Text>
            <Text
              style={[spamStyles.panelSubtitle, { color: colors.textMuted }]}
            >
      ...
      {distStatus && (
        <View
          style={[spamStyles.helpSection, { borderBottomColor: colors.border }]}
        >
          <View style={spamStyles.helpHeader}>
            <Ionicons name="people" size={16} color={Colors.accent} />
            <Text style={[spamStyles.helpTitle, { color: colors.text }]}>
              Help Scan
            </Text>
          </View>
          <Text style={[spamStyles.helpDesc, { color: colors.textMuted }]}>
            Your device fetches comments and the server detects spam. All
            results are verified server-side.
          </Text>

          <View style={spamStyles.statsRow}>
            <View style={spamStyles.statItem}>
              <Text style={[spamStyles.statValue, { color: colors.text }]}>
                {(distStatus.totalScanned || 0).toLocaleString()}
              </Text>
              <Text style={[spamStyles.statLabel, { color: colors.textMuted }]}>
                Scanned
              </Text>
            </View>
            <View style={spamStyles.statItem}>
              <Text style={[spamStyles.statValue, { color: Colors.danger }]}>
                {(distStatus.totalSpam || 0).toLocaleString()}
              </Text>
              <Text style={[spamStyles.statLabel, { color: colors.textMuted }]}>
                Spam
              </Text>
            ...
            <View style={spamStyles.statItem}>
              <Text style={[spamStyles.statValue, { color: Colors.accent }]}>
                {distStatus.activeWorkers || 0}
              </Text>
              <Text style={[spamStyles.statLabel, { color: colors.textMuted }]}>
                Workers
              </Text>
          ...
          {scanError && (
            <Text
              style={[
                spamStyles.scanError,
                {
                  color: scanError.includes("complete")
                    ? Colors.accent
                    : Colors.danger,
                },
              ]}
          ...
          {authToken ? (
            <Pressable
              style={[
                spamStyles.helpBtn,
                scanRunning && spamStyles.helpBtnStop,
              ]}
            ...
            <Text
              style={[spamStyles.helpLoginMsg, { color: colors.textMuted }]}
            >
              ...
              <View>
                ...
                >
                  <View style={spamStyles.groupInfo}>
                    <Text
                      style={[spamStyles.groupPreview, { color: colors.text }]}
                      numberOfLines={2}
                    >
                  ...
                  <View style={spamStyles.groupRight}>
                    <View style={spamStyles.countBadge}>
                      <Text style={spamStyles.countText}>
                    ...
                    {loadingExpand ? (
                      ...
                      expandedComments.slice(0, 20).map((c: any) => (
                        <View
                          key={c.comment_id}
                          style={[
                            spamStyles.expandedRow,
                            { borderBottomColor: colors.border },
                          ]}
                        >
                          <Text
                            style={[
                              spamStyles.expandedId,
                              { color: colors.textMuted },
                            ]}
                          >
                            #{c.comment_id}
...
export default function CommentsScreen() {
  ...
  const recomputeSpam = useCallback(
  ...
  const loadComments = useCallback(
    async (refresh = false) => {
      ...
      try {
        ...
        let skipped = 0;
        let cursor: string | undefined = refresh
          ? undefined
  ...
  const renderComment = useCallback(
    ({ item }: { item: PostComment }) => {
      ...
      const displayName =
        user?.displayName || user?.userName || `User #${item.userId}`;
          ...
          <View style={styles.cardBody}>
            ...
            {item.parentId && (
              ...
              >
                ...
                />
                <Text
                  style={[styles.replyIndicatorText, { color: colors.accent }]}
                >
                  Reply to a comment
                </Text>
              </View>
            ...
            <View style={[styles.footer, { borderTopColor: colors.border }]}>
              <View style={styles.footerChip}>
                <Ionicons
                  name="heart"
                  size={14}
                  color={item.likes > 0 ? Colors.like : colors.textMuted}
                />
                <Text
                  style={[
                    styles.footerText,
              ...
              <View style={styles.footerChip}>
                <Ionicons
                  name="heart-dislike"
                  size={14}
                  color={item.dislikes > 0 ? Colors.danger : colors.textMuted}
                />
                <Text
                  style={[
                    styles.footerText,
                    {
                      color:
                        item.dislikes > 0 ? Colors.danger : colors.textMuted,
                    },
                  ]}
                >
                  {item.dislikes}
              ...
              >
                <Text style={[styles.viewPostText, { color: colors.accent }]}>
                  View Post
                </Text>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      {(spamCount > 0 || serverSkipped > 0) && (
        ...
        >
          {spamCount > 0 && (
            ...
            >
              <Ionicons
                name={showSpam ? "eye-off" : "eye"}
                size={14}
                color={Colors.accent}
              />
              <Text
                style={[styles.spamBannerText, { color: colors.textSecondary }]}
              >
      ...
      {spamPanelOpen && (
        <SpamPanel
          colors={colors}
          onClose={() => setSpamPanelOpen(false)}
          authToken={authToken}
        />
      )}
      <FlatList
